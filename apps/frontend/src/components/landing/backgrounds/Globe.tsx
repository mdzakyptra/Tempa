...
function Tooltip({
  ...
  const visible = anchor.replace("--cobe-", "--cobe-visible-");
  return (
    <div
      style={{
        position: "absolute",
        ...
        whiteSpace: "nowrap",
        pointerEvents: "none",
        boxShadow: shadow ? "0 1px 4px rgba(0,0,0,0.1)" : undefined,
...
export function Globe({
  ...
  return (
    <div className={`relative aspect-square select-none ${className}`}>
      ...
      />
      {markers.map((m) => (
        <Tooltip key={m.id} anchor={`--cobe-${m.id}`} bg="#1a1a2e" fg="#fff">
          {m.label}
        </Tooltip>
      ))}
      {arcs
        .filter((a) => a.label)
        .map((a) => (
          <Tooltip key={a.id} anchor={`--cobe-arc-${a.id}`} bg="#fff" fg="#1a1a2e" shadow>
            {a.label}
          </Tooltip>
        ))}
    </div>
